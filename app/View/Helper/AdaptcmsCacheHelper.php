 ...
 */
class AdaptcmsCacheHelper extends AppHelper {
	...
	protected function _parseFile($file, $cache) {
		...
		$file = $this->_View->_parse($cache, $file);
	...
	protected function _writeFile($content, $timestamp, $useCallbacks = false) {
		...
		$this->_View->viewVars = $this->_View->_findBlocks($content, $this->_View->viewVars, false);
		...
		$file .= '
				...
				$this->View->setRoutes(\'' . json_encode( Configure::read('current_routes') ) . '\');
