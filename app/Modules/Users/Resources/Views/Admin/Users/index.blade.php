	...
	<div class="ui stackable center aligned grid">
		<div class="ui four wide column fluid users search">
		  <div class="ui icon input">
		    <input id="keyword" class="prompt" type="text" placeholder="Search users...">
	  ...
	  <div class="ui right floated right aligned ten wide column index-nav">
		  ...
		  <div class="ui cms dropdown">
		    ...
		    <div class="menu">
			    <div class="{{ !Request::get('role_id') ? 'active' : '' }} item">
				    <a href="{{ route('admin.users.index', [ 'role_id' => '', 'status' => Request::get('status', 1) ]) }}">Any</a>
			    </div>

			    @foreach($roles as $role)
			    	<div class="{{ Request::get('role_id') == $role->id ? 'active' : '' }} item">
				    	<a href="{{ route('admin.users.index', [ 'role_id' => $role->id, 'status' => Request::get('status') ]) }}">{{ $role->name }}</a>
			    	</div>
			    @endforeach
		    </div>
		  </div>
		  ...
		  <div class="ui cms dropdown">
		    ...
		    <div class="menu">
			    <div class="{{ Request::get('status', 1) === '' ? 'active' : '' }} item">
				    <a href="{{ route('admin.users.index', [ 'role_id' => Request::get('role_id'), 'status' => '' ]) }}">Any</a>
			    </div>

			    <div class="{{ Request::get('status', 1) === 1 ? 'active' : '' }} item">
				    <a href="{{ route('admin.users.index', [ 'role_id' => Request::get('role_id'), 'status' => 1 ]) }}">Active</a>
			    </div>
			    <div class="{{ Request::get('status', 1) === '0' ? 'active' : '' }} item">
				    <a href="{{ route('admin.users.index', [ 'role_id' => Request::get('role_id'), 'status' => 0 ]) }}">Pending</a>
			    </div>
		    </div>
		  </div>
			...
			<tbody>
				@foreach($items as $item)
					<tr>
						...
						<td>
							<a href="{{ route('admin.users.edit', [ 'id' => $item->id ]) }}"><strong>{{ $item->username }}</strong></a>
